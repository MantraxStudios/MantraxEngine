        ...
        if (treeNodeOpen)
        {
            ...
            if (ImGui::BeginPopup("AudioSourceOptions"))
            {
                ...
                if (ImGui::MenuItem("Reset"))
                {
                    ...
                    audioSource->setSound("", true, false, false);
            ...
            if (!removeComponent && audioSource != nullptr)
            {
                ...
                std::string currentPath = audioSource->getSoundPath();
                strncpy_s(soundPathBuffer, currentPath.c_str(), sizeof(soundPathBuffer) - 1);

                ImGui::Text("Sound File:");
                if (ImGui::InputText("##SoundPath", soundPathBuffer, sizeof(soundPathBuffer)))
                {
                    audioSource->setSound(std::string(soundPathBuffer),
                                          audioSource->is3DEnabled());
                }

                // Drag and drop target for audio files (manteniendo tu lógica original)
                if (ImGui::BeginDragDropTarget())
                {
                    if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("AudioClass"))
                    {
                        std::string droppedFile = std::string(static_cast<const char *>(payload->Data));
                        audioSource->setSound(droppedFile, audioSource->is3DEnabled());
                    }
                ...
                if (ImGui::Checkbox("3D Spatial Sound", &is3D))
                {
                ...
                if (audioSource->is3DEnabled())
                {
                    ...
                    if (ImGui::DragFloat("##MinDistance", &minDistance, 0.1f, 0.1f, 1000.0f, "%.1f"))
                    {
                    ...
                    if (ImGui::DragFloat("##MaxDistance", &maxDistance, 1.0f, minDistance + 0.1f, 10000.0f, "%.1f"))
                    {
                    ...
                    // Mostrar posición actual del objeto
                    if (go)
                    {
                ...
                bool isPaused = audioSource->isPaused();

                if (isPlaying && !isPaused)
                {
                    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.8f, 0.3f, 0.3f, 1.0f)); // Rojo para Stop
                    if (ImGui::Button("Stop##AudioSource"))
                    {
                        audioSource->stop();
                        std::cout << "UI: Deteniendo audio" << std::endl;
                    }
                    ImGui::PopStyleColor();
                }
                else
                {
                    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.3f, 0.8f, 0.3f, 1.0f)); // Verde para Play
                    if (ImGui::Button("Play##AudioSource"))
                    {
                        std::cout << "UI: Iniciando reproducción de: " << audioSource->getSoundPath() << std::endl;
                        std::cout << "UI: Configuración - 3D: " << (audioSource->is3DEnabled() ? "Sí" : "No")
                                  << ", Volumen: " << audioSource->getVolume() << std::endl;
                        audioSource->play();
                    }
                    ImGui::PopStyleColor();
                }
                ...
                if (isPaused)
                {
                    ...
                    {
                        audioSource->resume();
                        std::cout << "UI: Reanudando audio" << std::endl;
                    }
                }
                else
                {
                    ...
                    {
                        audioSource->pause();
                        std::cout << "UI: Pausando audio" << std::endl;
                    }
                }
                ...
                if (!audioSource->getSoundPath().empty())
                {
                    ImGui::Text("File: %s", audioSource->getSoundPath().c_str());
                }
                else
                {
                    ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.0f, 1.0f), "No audio file selected!");
                }
            }
            ImGui::TreePop();
        }
        ...
        if (treeNodeOpen)
        {
            ...
            if (!removeComponent && lightComp != nullptr)
            {
                ...
                const char *lightType =
                    lightComp->getType() == LightType::Directional ? "Directional Light" : lightComp->getType() == LightType::Point ? "Point Light"
                                                                                                                                    : "Spot Light";
    ...
    if (auto physicalObject = go->getComponent<PhysicalObject>())
    {
        ...
        if (treeNodeOpen)
        {
            ...
            if (!removeComponent && physicalObject != nullptr)
            {
                ...
                if (state->collisionMaskStatic)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Static Objects");
                if (state->collisionMaskDynamic)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Dynamic Objects");
                if (state->collisionMaskTrigger)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Triggers");
                if (state->collisionMaskPlayer)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Player");
                if (state->collisionMaskEnemy)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Enemy");
                if (state->collisionMaskProjectile)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Projectiles");
                if (state->collisionMaskSensor)
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.2f, 1.0f), " Sensors");
    ...
    if (auto cc = go->getComponent<CharacterController>())
    {
        ...
        if (treeNodeOpen)
        {
            ...
            if (!removeComponent)
            {
                ...
                if (ImGui::DragFloat("Walk Speed", &walk, 0.1f, 0.0f, 20.0f))
                    cc->setWalkSpeed(walk);

                float run = cc->getRunSpeed();
                if (ImGui::DragFloat("Run Speed", &run, 0.1f, 0.0f, 20.0f))
                    cc->setRunSpeed(run);

                float crouch = cc->getCrouchSpeed();
                if (ImGui::DragFloat("Crouch Speed", &crouch, 0.1f, 0.0f, 20.0f))
                    cc->setCrouchSpeed(crouch);

                float jump = cc->getJumpForce();
                if (ImGui::DragFloat("Jump Force", &jump, 0.1f, 0.0f, 20.0f))
                    cc->setJumpForce(jump);

                float grav = cc->getGravity();
                if (ImGui::DragFloat("Gravity", &grav, 0.1f, -30.0f, 30.0f))
                    cc->setGravity(grav);

                float air = cc->getAirControl();
                if (ImGui::DragFloat("Air Control", &air, 0.01f, 0.0f, 1.0f))
                    cc->setAirControl(air);
